 ...
 *  2. **Settings pages about Flint** ([SETTINGS_PACKAGES]) — blocked when the visible text
 *     mentions Flint (by label or package name) AND the window looks dangerous: its class
 *     name hints at an App-info or Accessibility screen ([PROTECTED_CLASS_HINTS]), or a
 *     danger phrase ([DANGER_PHRASES]) is visible. Plain Settings use (Wi-Fi, the all-apps
 *     list where "Flint" is merely a row) stays reachable during a block. When the window
 *     text is unreadable, the class name alone decides, fail-closed on the dangerous
 *     families — see [isProtectedSettingsSurface].
 *
...
class UninstallGuard {
    ...
    /** True for surfaces the guard may want to judge — gate the node-text collection. */
    fun isCandidateSurface(packageName: String): Boolean =
        packageName in UNINSTALLER_PACKAGES || packageName in SETTINGS_PACKAGES

    /**
     * True while any enabled HARDCORE rule's window is active — the cheap pre-gate callers
     * check before paying for a window-text sweep. [judge] re-derives the same answer, so
     * skipping the sweep when unarmed never changes a verdict.
     */
    fun isArmed(
        ...
        weekday: Int = -1,
        nowEpochMs: Long = -1L,
    ): Boolean = activeHardcoreRule(activeRules, nowMinutesOfDay, weekday, nowEpochMs) != null
    ...
    fun judge(
        ...
        val hardcore = activeHardcoreRule(activeRules, nowMinutesOfDay, weekday, nowEpochMs) ?: return null
        return when {
    ...
    /** First enabled, unexpired HARDCORE rule whose schedule window is active now, else null. */
    internal fun activeHardcoreRule(
        ...
        it.enabled &&
            !it.isExpired(nowEpochMs) &&
            it.breakLevel == BreakLevel.HARDCORE &&
            schedules.scheduleActive(it.schedule, nowMinutesOfDay, weekday)
    }

    /**
     * A Settings window is protected when it is *about Flint* (label or package name visible)
     * and *dangerous* (App-info / Accessibility class hint, or a visible danger phrase) —
     * both gates keep ordinary Settings usable during a block.
     *
     * When the window text is **unreadable** (empty sweep — a stale `rootInActiveWindow`
     * during the window transition, or an OEM tree that exposes nothing), the mention gate
     * has nothing to judge, so the class name decides alone: **fail closed** on the dangerous
     * screen families (mirroring iOS `denyAppRemoval`'s device-wide denial — another app's
     * App-info may briefly shield too, accepted), and open elsewhere so Wi-Fi/battery/etc.
     * stay reachable.
     */
    internal fun isProtectedSettingsSurface(
        ...
        val classProtected = PROTECTED_CLASS_HINTS.any { className.contains(it) }
        if (surface.visibleText.isEmpty()) return classProtected
